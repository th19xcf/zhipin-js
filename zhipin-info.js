...
(function() {
    'use strict';
    // -------------------- 全局常量定义 --------------------
    const SCRIPT_VERSION = '8.6.7'; // 更新版本号
    ...
    const PATTERNS = {
        age: /(\d{1,2})\s*岁/,
        experience: /(?:(\d{1,2})岁)?\s*(\d{1,2}年(?:经验|应届生|毕业)?|应届生|多年经验|10年以上|1年以内|在校生)?/i,
            ...
            if (detailRoot) {
                ...
                for (const infoText of allInfoTexts) {
                    ...
                    if (experience === 'N/A') {
                        ...
                        if (infoText.match(/(\d{1,2}年\s*应届生)/)) {
                            fullMatch = infoText.match(/(\d{1,2}年\s*应届生)/);
                            experience = fullMatch[0].trim();
                        } else if (infoText.match(/(\d{1,2}年\s*毕业)/)) {
